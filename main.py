        ...
        try:
            while True:
                ...
                if mean_rgb is not None:
                    ...
                    latest_rppg = self.overlap_add_latest()
                    if latest_rppg is not None:
                        self.rppg_buffer.append(latest_rppg)
                    else:
                        self.rppg_buffer.append(0.0)

                    self.trim_buffers()

                    if now - last_bpm_update >= 1.0:
                        if len(self.rppg_buffer) > 10:
                            fs = self.get_fps()
                            bpm_est = self.estimate_hr_from_rppg(list(self.rppg_buffer), fs)
                            if bpm_est is not None:
                                if current_bpm is None:
                                    current_bpm = bpm_est
                                else:
                                    current_bpm = 0.85 * current_bpm + 0.15 * bpm_est

                                self.bpm_timestamps.append(now)
                                self.bpm_buffer.append(current_bpm)

                        last_bpm_update = now

                display = self.draw_info(frame, face_box, mask, current_bpm)
                cv2.imshow("Real-time rPPG + Heart Rate (POS) - Pro UI", display)
